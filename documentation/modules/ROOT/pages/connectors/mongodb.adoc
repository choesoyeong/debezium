...
The connector then uses the change stream `documentKey`, which for a sharded collection consists of the shard key followed by `_id`.
...
----
  {
    "payload": {
      "id": "{\"caseNo\": \"201907130000200001\", \"_id\": {\"$oid\": \"5d2974673484856dfa2b909a\"}}"
    }
  }
----
...
Do not combine `document_key` with the {link-prefix}:{link-mongodb-event-flattening}[event flattening SMT] yet.
Where that SMT copies the record key into the `_id` field of the payload, which it does for rewritten deletes and for partial updates, it writes the whole document key rather than the `_id` that the key carries.
====
